...
> 앞서 작성된 글 - [운영 로그와 디버그 로그 분리하기](https://jojoldu.tistory.com/773)을 먼저 보면 좋다.
...
서비스의 규모가 커짐에 따라 사용자의 각종 이벤트 지표를 전달하는 행위는 많아지며,  
...
> [Mixpannel](https://mixpanel.com/home/) 및 [GTM - Google Tag Manager](https://marketingplatform.google.com/intl/ko/about/tag-manager/)과 같은 지표 전송 도구들은 실제로는 외부 서비스이다.  
...
렌더링 로직 14 줄을 제외하면 나머지 30여줄이 모두 **지표 전송, 로깅 코드**이다.  
...
- **함수 캡슐화로 인한 인지 부하 감소**
  - 지표 전송 코드를 함수로 추출함으로써 지표 전송 코드를 캡슐화할 수 있게 되었다.
  - Mixpanel 로 보내던 지표를 Amplitude 와 같은 **다른 트래킹 도구로 교체한다고 해서 메인 비즈니스 로직을 수정할 필요가 없다**.
...
Props를 통한 의존성 주입의 장점
- 명시적인 의존성
  - 컴포넌트가 필요로 하는 의존성이 Props를 통해 명시적으로 전달되므로, 의존성 관리가 명확해진다.
- 재사용성 및 테스트 용이성
  - 다양한 환경에서 다른 구현체를 Props로 전달할 수 있어, 컴포넌트의 재사용성이 높아지고, 테스트가 용이해진다.

반대로 다음과 같은 단점들이 있다.

- Props 드릴링
  - 깊은 컴포넌트 트리에서 의존성을 전달해야 할 경우, 여러 레벨에 걸쳐 Props를 전달해야 하는 "Props 드릴링" 문제가 발생할 수 있다.
- 대규모 애플리케이션 관리
  - 애플리케이션의 규모가 커질수록, 의존성을 관리하기 위해 Props를 사용하는 것이 더 복잡해질 수 있다.

구체적으로 다음과 같은 이유들이 있다.

컴포넌트 재사용성 저하
- 특정 함수나 메서드에 의존하는 컴포넌트는 해당 함수나 메서드 없이는 재사용하기 어렵다. 
- 이는 컴포넌트의 범용성과 재사용성을 저하시킬 수 있다.

컴포넌트 간 결합도 증가
- 함수나 메서드를 직접 Props로 전달하면, 컴포넌트 간 결합도가 증가한다. 
- 이는 나중에 로직 변경이 필요할 때 여러 컴포넌트를 함께 수정해야 하는 상황으로 이어질 수 있다.
