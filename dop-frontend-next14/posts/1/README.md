...
명시적인 의존성: 컴포넌트가 필요로 하는 의존성이 Props를 통해 명시적으로 전달되므로, 의존성 관리가 명확해진다.
재사용성 및 테스트 용이성: 다양한 환경에서 다른 구현체를 Props로 전달할 수 있어, 컴포넌트의 재사용성이 높아지고, 테스트가 용이해진다.

반대로 
Props 드릴링: 깊은 컴포넌트 트리에서 의존성을 전달해야 할 경우, 여러 레벨에 걸쳐 Props를 전달해야 하는 "Props 드릴링" 문제가 발생할 수 있다.
대규모 애플리케이션 관리: 애플리케이션의 규모가 커질수록, 의존성을 관리하기 위해 Props를 사용하는 것이 더 복잡해질 수 있다.
